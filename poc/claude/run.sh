#!/bin/bash
# CSP POC - Claude Chat Launcher
# Creates 2-pane tmux: top=chat, bottom=Claude

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SESSION_NAME="csp-claude-poc"
...
if ! command -v jq &> /dev/null; then
    echo "Error: jq not found (required for hooks)"
    exit 1
fi
...
# Clean up temp files
rm -f /tmp/csp-claude-start /tmp/csp-claude-end /tmp/csp-claude-response

# Create new session
echo -e "${YELLOW}Creating tmux session...${NC}"
unset TMUX
tmux new-session -d -s "$SESSION_NAME" -n "claude-poc"

# Split: top=chat (25%), bottom=claude (75%)
...
tmux select-pane -t "$AGENT_PANE" -T "Claude"

# Launch Claude in bottom pane
echo -e "${YELLOW}Launching Claude...${NC}"
tmux send-keys -t "$AGENT_PANE" "claude --dangerously-skip-permissions" Enter
